...
modes = c("plain", "complex")

multifunctionality <- c("plain" = "mono", "complex" = "multi")
...
for (aft in simp$mdata$aftNames) {
			...
			for (giStage in giStages) {
				for (guStage in guStages) {	
					...
					d["productionCsvFile"] <- paste(simp$batchcreation$versiondirs$production, 
							"/production/", aft, "/AftProduction_", aft, "_", multifunctionality[mode], "_", 
